# ⚛️ Explaining `const root = ReactDOM.createRoot(document.getElementById("root"));` in Depth for Beginners
...
## 1️⃣ `document.getElementById("root")`
...
## 2️⃣ `ReactDOM.createRoot(...)`

* ❓ **What is `ReactDOM`?**

  * ⚛️ React has two main parts:
...
* ❓ **What does `createRoot()` do?**
  ...
  * 🕰️ Before React 18, React used `ReactDOM.render()` to show your app inside the webpage. But React 18 introduced a new method called `createRoot()`.
  * 🏗️ `createRoot()` sets up **a special root container** where React will manage your app’s user interface.
  * ⚙️ It prepares the place (the DOM element you gave it, here `document.getElementById("root")`) to be controlled by React using React’s latest and fastest rendering technology.
...
## 3️⃣ Putting it together: `const root = ReactDOM.createRoot(document.getElementById("root"));`

* 📍 You find your webpage’s container (the `<div id="root">`),

* 🏗️ then create a React “root” inside that container — React’s way of managing everything inside that container.

* ℹ️ This line **does not display anything yet** — it just sets up the root.

* ▶️ To actually show something, you call:

  ```js
  root.render(<App />);
  ```

* 🖥️ That tells React: “Render the `<App />` component inside this root container.”
...
## ⚙️ What happens under the hood when you call `createRoot`?

* 🏗️ React initializes an internal data structure to keep track of your app’s components and UI state.
...
| ⚛️ `ReactDOM.createRoot()`           | Sets up React’s modern system to manage and update that place.     |
