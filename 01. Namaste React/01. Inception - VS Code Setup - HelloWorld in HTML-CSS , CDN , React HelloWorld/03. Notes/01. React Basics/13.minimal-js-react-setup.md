...
* ⚛️ React: minimal setup includes linking React and ReactDOM scripts via CDN in HTML.

---

# 🚀 Minimal JavaScript and React Programs with Setup

### 1️⃣ Shortest JavaScript Program
...
* 🖨️ This prints `"Hello"` to the console.
* 📄 An empty `.js` file also runs without error but does nothing.

---

### 2️⃣ Shortest React Program (Functional Component)
...
* ⚛️ This defines a simple React component that renders a `<div>` with the text `"Hello"`.

---

### 3️⃣ Minimal React Setup Using CDN (No Build Tools)
...
* 🏗️ The React element is created with `React.createElement`.
* 🖥️ ReactDOM renders the element inside the `#root` div.
...
| ⚛️ Shortest React program | Functional component returning JSX            | `const App = () => <div>Hello</div>;` |
